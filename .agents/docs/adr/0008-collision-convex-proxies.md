...
- **The proxy carries no mass.** Mass, centre of mass, and angular inertia are *authored* on the hull (RON spec + the `Center_Of_Mass` empty) and marked `NoAuto*`, so the proxy is collision-only — its volume/centroid never set the tank's weight or balance ([[0010-per-variant-data-in-ron]], [[0011-required-model-contract-fails-fast]]). This is deliberate: the proxy is an abstract envelope and says nothing about weight distribution.
